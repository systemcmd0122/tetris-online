...
export const UPDATE_LOG = [
  {
    version: "3.4.0",
    title: "ゲームエンジン刷新・操作性向上・管理機能強化",
    changes: [
      "テトリスエンジンを完全に再設計し、SRS（Standard Rotation System）に準拠した正確な回転と壁蹴り（Wall Kick）を実装しました。",
      "3コーナー法によるT-Spin判定、Comboシステム、Back-to-Backボーナスなどの標準的な競技ルールを正確に実装しました。",
      "入力システムを刷新。DAS（Delayed Auto Shift）とARR（Auto Repeat Rate）を導入し、遅延のないプロ仕様の操作感を実現しました。",
      "ゴースト（落下予測位置）表示を追加。常に正確な着地地点が把握できるようになりました。",
      "スマートフォン・タブレットでのプレイに完全対応。タッチ操作の最適化とレスポンシブUIにより、端末を問わず快適にプレイ可能です（「PC専用」表示を廃止）。",
      "管理コンソールの観戦機能を大幅強化。スプリントを含む全モードで、ホールド・ネクスト・ゴースト・盤面状態を完全リアルタイムで監視可能になりました。",
      "マルチ対戦の同期ロジックを改善。お邪魔ブロックの計算をアトミック化し、通信遅延による同期ズレを最小限に抑えました。",
      "描画エンジンを最適化。メモリリークの解消と描画負荷の軽減により、長時間プレイ時の安定性が向上しました。"
    ]
  },
  {
    version: "3.3.1",
